            ...
            if app.previous_intent == "StrangerIntent":
                try:
                    ...
                    if app.name_count < 3:
                        ...
                        response['response']['outputSpeech']['values']['value'] = '不審者発見'
                        try:
                           line_bot_api.push_message('U27380939fd4e86861e37c0e0f94a90ec', TextSendMessage(text='不審者です'))
                        except LineBotApiError as e:
                            print('error')
                        app.name_count = 0
            else:
                response['response']['outputSpeech']['values']['value'] = 'もう一度おねがい'
